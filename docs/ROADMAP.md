...
| M3 — Platform slice | 3 nodes → gateway → MQTT → TimescaleDB → Grafana, one alert firing | ◐ live via HTTPS transport (simulated sensor → Timescale hypertable → rule → alert → Grafana, smoke-tested on the composed stack); MQTT transport and real nodes remain |
