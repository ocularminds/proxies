...
| P2.2 ◐ | Mosquitto in the one-box compose (anonymous on-box listener; config documents the lock-down path — TLS, per-device credentials/mTLS, topic ACLs — as deployment policy since the signed envelope carries the trust) |
| P2.3 ✅ | MQTT ingest bridge: subscribes `proxies/telemetry/+`, runs the same transport-agnostic pipeline as HTTPS (extracted to `ingest.ts`), acks verdicts on `proxies/telemetry-ack/<id>`; integration-tested against an in-process broker |
...
| P2.5 ◐ | Threshold rules v1: org/site/device-scoped `gt`/`lt` rules evaluated at ingest, at most one alert per rule per batch; alerts persisted with delivery status; pluggable notifier (webhook + console today — SMS/WhatsApp adapters and rate-of-change rules are the follow-up) |
| P2.6 ✅ | Fleet health: `GET /admin/fleet?organization=` lists every device (kind, site, owner, latest battery from telemetry) and host with `online` / `stale` / `never-seen` staleness (`staleAfterMs` tunable). Firmware tracking arrives with OTA (gateway, P2.7) |
...
| P2.8 ✅ | One-box stack booted and smoke-tested end to end (2026-08-17): compose up → all 10 migrations applied on boot, `telemetry` confirmed a real hypertable, Grafana provisioned; live smoke ran site → sensor enrollment (real Ed25519) → signed batches → seq replay rejected → rule fired on a dry-soil reading → alert delivered → fleet online |
| P2.9 ✅ | HTTPS batch ingest (pulled forward as the first transport): `POST /telemetry` accepts Ed25519-signed batches from any enrolled endpoint; strictly monotonic per-device `seq` claimed atomically (replays/reorders 409), stale-batch and future-reading guards; sensors/gateways enroll site-bound via the extended admin API |
| P2.10 ✅ | Presence-as-telemetry: every approved validation writes `{type: presence, value: 1}` for the device at the attesting host's site — the original product is now the platform's first stream, and rules/alerts apply to it like any sensor metric |
...
| PR | Feature |
| --- | --- |
| P3.0 ✅ | **All four verticals shipped as declarative kits** (`server/src/kits/`): metric catalogs with typical ranges, labeled default rule packs, field notes. `GET /kits`, `GET /kits/:key`, and idempotent `POST /admin/sites/:id/apply-kit` instantiate a kit's rules per site. Templated **Vertical Kit** Grafana dashboard (metric + site variables, labeled alerts). `npm run simulate -- --kit <key> --site <id> --admin <token>` streams plausible signed readings (with optional `--breach`) so any pilot can be rehearsed before hardware exists |
| P3.W1 ✅ | **Waste pilot activation** (vertical chosen by owner, 2026-08-17): `npm run provision` creates site + applies kit + issues N bins with a field manifest (JSON/CSV); `GET /admin/routes/waste` returns the collection route (bins ≥ threshold, fullest first) — the vertical's core product; [docs/PILOTS/waste.md](PILOTS/waste.md) runbook covers per-bin NB-IoT BOM, ultrasonic fill calibration with on-device median filtering, install flow, and 30-day M4 acceptance criteria |
| P3.W2 ◐ | **Reference firmware shipped and CI-compiled** (`firmware/waste-bin/`, ESP32/PlatformIO): Ed25519 identity in NVS, code enrollment, median-of-5 ultrasonic fill with per-bin calibration, canonical-JSON signing with JS-compatible number formatting, monotonic seq + store-and-forward across deep sleep (409-after-lost-ack handled), bench mode (WiFi, 30 s) vs field mode (`#error`-marked TinyGSM cellular swap point). Remaining: hardware procurement, tilt/temp/battery hooks for the chosen board, install, 30-day run → M4 |
...
| M3 — Platform slice | 3 nodes → gateway → MQTT → TimescaleDB → Grafana, one alert firing | ◐ both transports live-verified on the composed stack (HTTPS: agri scenario with rule + alert; MQTT: bin-fill batch through real mosquitto into the hypertable); only real hardware nodes/gateway remain |
...
| 2026-08-17 | P3.W2 (firmware half) shipped: the reference bin-node firmware compiles to a real ESP32 image (verified locally via PlatformIO in a sandboxed toolchain, and now in CI). It implements the full envelope: NVS-held Ed25519 identity, one-time-code enrollment, median-of-5 ultrasonic sampling, canonical JSON with JSON.stringify-compatible number formatting (the signature-breaking sharp edge, documented), monotonic seq with store-and-forward across deep sleep, and a deliberate `#error` at the cellular swap point so field builds cannot silently ship the bench transport. |
| 2026-08-17 | **Waste chosen as the pilot vertical** (owner decision). P3.W1 shipped: provisioning CLI (site + kit + N enrollment codes → field manifest), the collection-route endpoint (`/admin/routes/waste` — latest fill per bin, threshold-filtered, fullest first), and the field runbook (per-bin NB-IoT architecture — no shared gateway for this vertical, narrowing P2.7 to per-bin firmware; ultrasonic calibration; M4 acceptance criteria). |
| 2026-08-17 | Phase 3 opened with P3.0: all four verticals exist as declarative kits — data, never forks. Metric catalogs (snake_case types, units, typical ranges), labeled rule packs (e.g. "Bin needs collection", "ISO 10816 zone D"), and field notes ship in `server/src/kits/`; applying a kit to a site instantiates its rules idempotently; a templated Grafana dashboard serves any vertical; and a simulator CLI streams signed readings per kit so pilots rehearse before hardware. The pilot itself awaits the owner's vertical decision. |
| 2026-08-17 | MQTT live-verified on the composed stack: a fresh sensor enrolled over the admin API published a signed bin-fill batch to real mosquitto and received the pipeline's 200 ack; the row landed in the hypertable. Compose gained `MQTT_PORT` override (host 1883 can be taken). M3 now lacks only real hardware nodes. |
| 2026-08-17 | P2.2/P2.3 shipped: MQTT transport. The telemetry pipeline was extracted into a transport-agnostic module (`ingest.ts`) shared by the HTTPS route and a new MQTT bridge (`mqtt.ts`) that subscribes to `proxies/telemetry/+` and publishes verdicts to `proxies/telemetry-ack/<id>`; mosquitto joins the one-box compose with a loudly-documented pilot config. Broker auth is deployment policy — the Ed25519 envelope + monotonic seq carry the trust. README refreshed to platform status. |
| 2026-08-17 | P2.8 completed: first boot of the composed stack succeeded — server applied all 10 migrations on boot, `telemetry` came up as a genuine Timescale hypertable, Grafana provisioned the datasource and dashboard. A live smoke then exercised the whole platform: site bootstrap, sensor enrollment with freshly generated Ed25519 keys, healthy + replayed + breaching signed batches, rule firing (soil < 20%), alert delivery, fleet health, dashboard presence. M3 is now live modulo MQTT and real hardware nodes. |
| 2026-08-17 | P2.8 scaffolded: `deploy/` one-box compose (Timescale + server + Grafana), server Dockerfile (two-stage, migrations on boot), pre-provisioned datasource and Proxies Overview dashboard (telemetry by metric, presence count, recent alerts, fleet last-seen — SQL matches the live schema). Honest status: dashboard JSON validated and every underlying command is individually tested, but the composed stack has not yet been booted — run `docker compose up` once before calling P2.8 done. |
| 2026-08-17 | P2.6 shipped: fleet health. One admin call surfaces the whole fleet — devices with kind/site/owner and their latest battery reading (lateral join on telemetry), hosts per site, each flagged online/stale/never-seen against a tunable window. This also delivers the liveness observability P1.6's WebSocket registry was for; P1.6 stays deferred until a push channel is actually needed. |
| 2026-08-17 | P2.10 shipped: presence is telemetry. Approved validations write `{type: presence, value: 1}` at the attesting host's site into the same store as every sensor reading — the strategic reframe from the original review is now literal, and threshold rules apply to presence like any other metric. |
| 2026-08-17 | P2.5 (v1) shipped: threshold rules and alerts. Rules scope org → site → device on a metric type with gt/lt thresholds; evaluation runs at ingest with per-rule-per-batch dedupe; alerts persist with delivery status; delivery is a pluggable notifier — webhook (3 s timeout) or loud console today, SMS/WhatsApp adapters next. Admin: `POST /admin/rules`, `GET /admin/alerts?organization=`. |
| 2026-08-17 | P2.9 shipped (pulled forward as the platform's first transport): signed HTTPS batch ingest. Sensors and gateways enroll site-bound through the same one-time-code flow as phones; `POST /telemetry` verifies device status, batch freshness, an atomically claimed strictly-monotonic `seq` (replay/reorder → 409), and the Ed25519 batch signature before multi-row insert with org/site attribution. MQTT (P2.2/P2.3) remains the constrained-link upgrade path. |
