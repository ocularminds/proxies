...
| P2.8 ◐ | Grafana provisioning + one-box compose scaffold (`deploy/`): Timescale + server (Dockerfile, migrations on boot) + Grafana with pre-provisioned datasource and the Proxies Overview dashboard (telemetry, presence, alerts, fleet). Dashboard JSON validated; **first `docker compose up` still pending** (no Docker daemon in the dev session) |
